...
comercioasia_tabela_divisao <- function(periodo) {

  tabela_prep <- Asia::comercioasia_dados_divisao(periodo)  %>%
    dplyr::ungroup() %>%
    ...
    dplyr::mutate(dplyr::across(dplyr::starts_with(c("var", "Imp", "Exp", "Cor")), scales::label_percent())) %>%
    ...
    dplyr::relocate(co_ano, .data$Exportacoes, .data$Importacoes, .data$Corrente) %>%
