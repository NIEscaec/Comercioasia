...
comercioasia_grafico_produtos_proporcao <- function(pais, periodo) {

  ano_max <- comercioasia_dados_produtos(pais, periodo) %>%
    dplyr::ungroup() %>%
  ...
  if (periodo == "mensal") {
    df <- Asia::comercioasia_dados_produtos(pais, periodo) %>%
      dplyr::ungroup() %>%
      dplyr::filter(co_ano == max(co_ano))

    frase <- paste0(Asia::comercioasia_get_ulimoano(), " at\u00e9 ", Asia::meses(barao2::comercioasia_get_ultimomes()))
  }
    ...
    frase <- paste0("em ", Asia::comercioasia_get_ulimoano()-1)
  }
  ...
  df %>%
    ...
    treemap::treemap(index = c("path", "no_sh4_por"),
                     ...
                     title = glue::glue("Asia, Propor\u00e7\u00e3o de Exporta\u00e7\u00f5es e Importa\u00e7\u00f5es {frase}"))
}
